...
class EnlistControllerIT extends AbstractControllerIT {
    ...
    void enlist_student_in_section() throws Exception {
        insertNewDefaultSectionWithCapacity(10);
        insertDefaultStudent();
        defaultStudentAct(ENLIST);
    ...
    void cancel_student_in_section() throws Exception {
        insertNewDefaultSectionWithCapacity(10);
        insertDefaultStudent();
        defaultStudentAct(CANCEL);
        assertNumberOfStudentsSuccessfullyEnlistedInDefaultSection(0);
    }
    ...
    private void insertDefaultStudent() {
    ...
    private void defaultStudentAct(UserAction action) throws Exception {
        Student student = studentRepository.findById(DEFAULT_STUDENT_NUMBER).orElseThrow(() -> new NoSuchElementException("No student w/ student num " + DEFAULT_STUDENT_NUMBER + " found in DB."));
        mockMvc.perform(post("/enlist").sessionAttr("student", student).param("sectionId", DEFAULT_SECTION_ID).param("userAction", action.name()));
    }
