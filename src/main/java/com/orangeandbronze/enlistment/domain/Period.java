...
public class Period {
    ...
    void checkOverlap(Period other, boolean isAboutFaculty) {
        if (this.startTime.isBefore(other.endTime) && this.endTime.isAfter(other.startTime) && isAboutFaculty) {
            throw new ScheduleConflictException("Faculty Schedule overlap between this: " + this + " & other: " + other);
        }
        else if (this.startTime.isBefore(other.endTime) && this.endTime.isAfter(other.startTime) && !isAboutFaculty) {
